...
export default function TeluguSpelling() {
  ...
  const saveProgress = async (exerciseId: number, isCorrect: boolean, hintsUsed: number = 0, timeSpent: number = 0) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/learning-progress/spelling', {
        method: 'POST',
        headers: {
  ...
  const resetProgress = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch('http://localhost:5000/api/learning-progress/reset/spelling', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('telugu-basics-token')}`
        }
